...
class EmailClient:
    ...
    def _infer_smtp_settings(self):
        try:
            ...
            raise ValueError(f'Provider `{self.provider}` is not an acceptable type. Please choose a provider from the following {tuple(SMTP_SETTINGS.keys())}')

        try:
            self.server_address = smtp_settings['server_address']
            self.port = smtp_settings['port'][self.connection_strategy]
        except KeyError as e:
            raise ValueError(f'Provider `{self.provider}` does not support `{self.connection_strategy}` connection strategy')
    ...
    def send_email(self):
        logger.info(f'Starting `{self.connection_strategy}` connection to `{self.provider} SMTP server...`')
